...
def measure_distance(segmented_image,segmented_arm_image,arm_spread_image,waist_image,image,metre_pixel_x,metre_pixel_y):
	...
	dist_tuple=dist1,dist2,dist3
	row = [((dist+dist_ans)/2),((dist4+dist5)/2)]
	print("Shoulder Length",(dist+dist_ans)/2)
	print("Sleeve Length", (dist4+dist5)/2)
	with open('csv/result.csv', 'a') as writeFile:
		writer = csv.writer(writeFile,lineterminator='\n')
		
		writer.writerow([(perimeter*10),(((dist+dist_ans)/2)*10),(((dist4+dist5)/2)*10)])
		writeFile.close()
	df=pd.read_csv("csv/result.csv")
	print(df.head())
	with open('csv/test.csv', 'a') as writeFile:
		writer = csv.writer(writeFile,lineterminator='\n')
		
		writer.writerow([(1),(155),(((dist+dist_ans)/2)*10),(((dist4+dist5)/2)*10),(perimeter*10)])
		writeFile.close()
	
def download_images(urls, username):
	for index, url in enumerate(urls):
		try: os.makedirs(os.path.join('img', username))
		except OSError: pass # already exists
		with open('img/' + username + '/image' + str(index+1) + '.jpg', 'wb') as f:
...
def main():
	...
	firebase = FireBase()
	urls = firebase.fetch_user_image_urls(username)
	download_images(urls, username)

	image = cv2.imread('img/' + username + '/image1.jpg')
	arm_spread_image = cv2.imread('img/' + username + '/image2.jpg')
	waist_image = cv2.imread('img/' + username + '/image3.jpg')
	...
	dp = pandas.read_csv('csv/bodys.csv')
	df = pd.DataFrame(dp)
	cols = [1,4,7,8]
	df = df[df.columns[cols]]
#print df.head()
#df = DataFrame(dp,columns=['Weight'])
#df.dropna()
#print df.head()
#labels = np.array(['Small', 'Medium', 'Large'])
#kmeans = KMeans(n_clusters=3,init=np.array([[101,193,434,388],[612,355,778,741],[1123,518,1123,1094]])).fit(df)
...
#y=np.array(pred_classes)
	...
	for sample in range(0,4):
		...
		km = KMeans(n_clusters=3, init=np.array([[101,193,434,388],[612,355,778,741],[1123,518,1123,1094]]), max_iter=1, n_init=1)  # , verbose=1)
		km.fit(df)
	...
	my_df.to_csv('csv/foo.csv', index=False)



	dp1 = pandas.read_csv('csv/myfile2.csv')
	d2 = pd.DataFrame(dp1)
	findL = [0, 1, 2]
	replaceL = ['Small', 'Medium', 'Large']
	col = 'Size'
	d2[col] = d2[col].replace(findL, replaceL)
	d2.to_csv('csv/final.csv', index=False)


	with open('csv/out1.csv', 'w') as csvfile:
	...
	d9 = pandas.read_csv('csv/myfile.csv')
	tr = pd.DataFrame(d9)
	d10 = pandas.read_csv('csv/test.csv')
...
#print model.score(X_train,X_test)
	...
	print(y_pred)
	n_pred = clf_gini.predict(T)
	prediction = pd.DataFrame(n_pred, columns=['predictions1']).to_csv('csv/prediction1.csv')

	data1 = pandas.read_csv('csv/prediction1.csv')
	data2 = pandas.read_csv('csv/test.csv')

	data2['Size'] = data1['predictions1']
	data2.to_csv('csv/output.csv', header=['Unnamed: 0','Weight ','Shoulder breadth ','Chest circumference ','Waist circumference','Size'])

	dp11 = pandas.read_csv('csv/output.csv')
	d22 = pd.DataFrame(dp11)
	findL = [0, 1, 2]
	replaceL = ['Small', 'Medium', 'Large']
	col = 'Size'
	d22[col] = d22[col].replace(findL, replaceL)
	d22.to_csv('csv/output.csv', index=False)

	# Save to Firebase	
	df_json = list(json.loads(d22.iloc[-1:,2:].to_json(orient='index')).items())[0][-1]
	print(df_json)
	firebase.save(df_json, username)
